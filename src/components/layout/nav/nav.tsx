      ...
      return (
        <li key={item.label}>
          <DropdownMenu  modal={false}>
            ...
            <DropdownMenuContent className="bg-dark-blue border-0 shadow-lg rounded-none mt-4 py-2 animate-in fade-in-0 zoom-in-95 duration-200">
              {item.items.map((subItem) => (
                <DropdownMenuItem key={subItem.href} asChild>
                  <a
                    href={subItem.href}
                    className="cursor-pointer text-white/80 no-underline hover:text-white transition-opacity py-2 hover:rounded-none "
                  >
                    {subItem.label}
                  </a>
                </DropdownMenuItem>
